...
const AccountStandard = () => {
    ...
    const premium_status = {premium:"y"}
    
    const handleClick = async e =>{
        e.preventDefault()
        user.premium = "y"
        await axios.put("http://localhost:8800/updatepremium/" + user.ID, premium_status)
        navigate("/premiumchange")
    }

    return <div>
    <button>
        <Link to ="/home/standard">Return to homepage</Link>
    </button>
    <button>
    ...
    <button onClick = {handleClick}>Become a Premium User</button>
    </div>
}
